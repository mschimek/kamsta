...
struct EdgeInterval {
  ...
  Edge max_edge;
  EdgeInterval()
      : min_edge{VID_UNDEFINED, VID_UNDEFINED}, max_edge{VID_UNDEFINED,
                                                         VID_UNDEFINED} {}
...
private:
  auto get_it_to_min_pe_entry(const Edge& edge) const {
    assert(get_min_column_or_sentinel(edge) ==
           grid_communicators_.get()
               .get_row_ctx()
               .rank()); // edge is requested on correct column
    const auto it = std::lower_bound(
        non_empty_edge_intervals_.begin(), non_empty_edge_intervals_.end(),
        edge, [](const EdgeIntervalPE& interval_pe, const Edge& edge) {
          return interval_pe.interval.max_edge < edge;
        });
    return it;
  }
...
public:
  ...
  VertexLocator()
      : rank{mpi::MPIContext{}.rank()},
        grid_communicators_{mpi::get_power_two_grid_communicators()} {};
  ...
  void init(Edge min_edge, Edge max_edge, const Edges& edges) {
    ...
    const auto& column_ctx = grid_communicators_.get().get_col_ctx();
    const auto& row_ctx = grid_communicators_.get().get_row_ctx();
    ...
    for (int i = 0;
      ...
      if (!interval.is_empty())
        non_empty_edge_intervals_.emplace_back(
            interval, i + (row_ctx.rank() * column_ctx.size()));
    }
    // edge intervals is already sorted by construction
    const static Edge sentinel_edge{VID_UNDEFINED, VID_UNDEFINED};
    const Edge min_edge_on_column =
        non_empty_edge_intervals_.empty()
            ? sentinel_edge
            : non_empty_edge_intervals_.front().interval.min_edge;
    const Edge max_edge_on_column =
        non_empty_edge_intervals_.empty()
            ? sentinel_edge
            : non_empty_edge_intervals_.back().interval.max_edge;
    const EdgeInterval interval_on_column{min_edge_on_column,
                                          max_edge_on_column};
    const auto column_edge_intervals =
        mpi::allgather(interval_on_column, row_ctx);
...
private:
  PEID get_min_column_or_sentinel(const Edge& edge, PEID sentinel = -1) const {
    const auto it = std::lower_bound(
        ...
        [](const EdgeIntervalColumn& interval_column, const Edge& edge) {
          return interval_column.interval.max_edge < edge;
        });
  ...
  template <typename Edges> void request_ghost_locations(Edges& edges) {
    ...
    auto dst_calculator = [&](const Edge& edge, const std::size_t&) {
      return get_min_column_or_sentinel(edge);
    };
    auto requests = twopass_alltoallv_openmp_special(edges, filter, transformer,
                                                     dst_calculator, row_ctx);
    auto reply_filter = False_Predicate{};
    auto reply_transformer = [&](const Edge& edge, const std::size_t&) {
      return process_requested_edge(edge);
    };
    auto reply_dst_calculator = [&](const Edge&, const std::size_t& i) {
      return requests.get_pe(i);
    };
    // SEQ_EX(ctx, PRINT_CONTAINER_WITH_INDEX(requests.buffer););
    auto replies = twopass_alltoallv_openmp_special(
        requests.buffer, reply_filter, reply_transformer, reply_dst_calculator,
        row_ctx);
    for (const auto& reply : replies.buffer) {
      const auto pe_split_status = PESplitStatus{reply.pe, reply.is_split};
  ...
  [[nodiscard]] bool has_predecessor_pe_same_vertex(
    ...
    if (pe_idx_in_edge_intervals > 0) {
      ...
      return predecessor_interval.interval.max_edge.get_src() == edge.get_src();
    }
    ...
    if (column_idx > 0) {
      ...
      return predecessor_column_interval.interval.max_edge.get_src() ==
             edge.get_src();
    }
    return false;
  }
  ...
  [[nodiscard]] bool has_successor_pe_same_vertex(
    ...
    if (pe_idx_in_edge_intervals + 1 < non_empty_edge_intervals_.size()) {
      const auto& successor_interval =
          non_empty_edge_intervals_[pe_idx_in_edge_intervals + 1];
      return successor_interval.interval.min_edge.get_src() == edge.get_src();
    }
    const auto& column_idx = get_own_entry_idx_in_column_edge_interval();
    if (column_idx + 1 < non_empty_column_edge_intervals_.size()) {
      const auto& successor_column_interval =
          non_empty_column_edge_intervals_[column_idx + 1];
      return successor_column_interval.interval.min_edge.get_src() ==
             edge.get_src();
    }
    return false;
  }
  ...
  int rank;
  std::reference_wrapper<mpi::PowerTwoGridCommunicators> grid_communicators_;
  ...
  std::size_t idx_own_entry_column_edge_interval_ = -1;
