...
def generate_jobfiles(path_to_suite, jobfile_dir, output_dir, path_to_exec, path_supermuc_load_config):
    suite = ExperimentSuite(path_to_suite)
    experiments = suite.to_single_experiments()
    for num_cores, experiments in experiments.items():
        jobfile = Jobfile(suite.name, output_dir, num_cores, jobfile_dir, path_to_exec, path_supermuc_load_config, suite.time_limit)
...
def main():
    ...
    path_to_supermuc_load_config = Path(os.getcwd()) / "supermuc_load_config.sh"
    ...
    supermuc_load_config_in_exp_dir = experiment_dir / "supermuc_load_config.sh"
    shutil.copyfile(path_to_supermuc_load_config, supermuc_load_config_in_exp_dir)

    jobfile_dir = experiment_dir / "jobfiles"
    jobfile_dir.mkdir(exist_ok=True, parents=True)
    output_dir = experiment_dir / "output"
    output_dir.mkdir(exist_ok=True, parents=True)
    generate_jobfiles(path_to_suite, jobfile_dir, output_dir, path_to_exec_in_exp_dir, supermuc_load_config_in_exp_dir)
