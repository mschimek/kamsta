...
def get_queue_horeka(num_cores):
    if num_cores <= (128):
        return "dev_cpuonly"
    else:
        return "cpuonly"
...
def get_num_compute_nodes_horeka(num_cores):
...
class Jobfile:
    def __init__(self, name, output_directory, num_cores, num_threads, jobfile_output_directory, path_to_exec, time_limit):
        ...
        jobfile_subs = {}
        jobfile_subs["nodes"] = get_num_compute_nodes_horeka(num_cores)
        jobfile_subs["time"] = time_limit
        self.job_queue = get_queue_horeka(num_cores)
        ...
        jobfile_subs["num_tasks"] = num_cores // num_threads

        jobfile_subs["num_mpi_procs"] = num_cores // num_threads
        jobfile_subs["num_cpus_per_mpi_proc"] = num_threads
        jobfile_subs["num_mpi_procs_per_node"] = 64 // num_threads
